...
int main(void)
{
	...
	// 1. enable HSEBYP IN RCC_CCR
#define RCC_CCR_OFFSET 0x00UL
#define RCC_BASE_ADDR 0x40023800UL
#define RCC_CCR_ADDR (RCC_BASE_ADDR+RCC_CCR_OFFSET)

	uint32_t * rcc_ccr_addr = (uint32_t *)RCC_CCR_ADDR;
	*rcc_ccr_addr |= (0x1<<18); //HSEBYP BIT ENABLED
	//2. HSEON BIT
	*rcc_ccr_addr |= (0x1<<16);

	//3. Check HSERDY BIT
	while (!(*rcc_ccr_addr & (1<<17)));
...
#define RCC_config_ADDR (RCC_BASE_ADDR+RCC_config_OFFSET)
	...
	*rcc_config_addr &= ~(0x1 <<22); //HSE SELECTED
	*rcc_config_addr |= (0x1 <<21); //HSE SELECTED


	//5. GPIO PA8 AS AF0 AND MCO1
